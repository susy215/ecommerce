...
class PushNotificationService:
    ...
    def send_admin_notification(
        ...
        """
        Envía una notificación a un administrador específico.
        Crea la notificación en BD y la envía por WebSocket.
        ...
        Returns:
            Instancia de NotificacionAdmin creada
        """
        try:
            ...
            # Enviar por WebSocket usando Channels
            from channels.layers import get_channel_layer
            from asgiref.sync import async_to_sync

            channel_layer = get_channel_layer()
            group_name = f'admin_notifications_{usuario.id}'
            ...
            logger.info(f'✅ Notificación admin enviada a {usuario.username}: {titulo}')
            return notification

        except Exception as e:
            logger.error(f'❌ Error enviando notificación admin a {usuario.username}: {e}')
            raise
    ...
    def send_to_all_admins(
        ...
        """
        Envía una notificación a todos los administradores y vendedores.
        ...
        admins = Usuario.objects.filter(rol__in=['admin', 'vendedor'], is_active=True)
        notifications = []

        for admin in admins:
            try:
                notification = self.send_admin_notification(
                    usuario=admin,
                    tipo=tipo,
                    titulo=titulo,
                    mensaje=mensaje,
                ...
                logger.error(f'Error enviando notificación a {admin.username}: {e}')
                continue

        logger.info(f'📤 Notificación enviada a {len(notifications)} administradores: {titulo}')
        return notifications

    def send_nueva_compra_admin(self, compra) -> List[NotificacionAdmin]:
        """
        Notifica a todos los admins sobre una nueva compra.
        ...
        return self.send_to_all_admins(
            tipo='nueva_compra',
            titulo=titulo,
            mensaje=mensaje,
            url=f'/admin/orders/{compra.id}/',
            datos={
                'compra_id': compra.id,
                'cliente_id': compra.cliente.id,
                'cliente_nombre': compra.cliente.nombre,
    ...
    def send_nuevo_pago_admin(self, compra) -> List[NotificacionAdmin]:
        """
        Notifica a todos los admins sobre un nuevo pago confirmado.
